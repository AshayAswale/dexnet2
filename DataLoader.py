...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes):
        ...
        # Generate data
        for i, ID in enumerate(list_IDs_temp):                        #Each ID is incremented to add a new ID array to the batch which we dont want. 
            # Store sample
            x1[i,] = np.load('DEXNET_DATA' + self.x1_IDs[thousands])['arr_0.npy'][ones]
            x2[i,] = np.load('DEXNET_DATA' + self.x2_IDs[thousands])['arr_0.npy'][ones]
